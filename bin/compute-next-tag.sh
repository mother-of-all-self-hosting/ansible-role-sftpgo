...
#
# Tags look like `v<SFTPGo version>-<release>`, which is what this repository
# has always published (v2.6.6-0 ... v2.7.5-0):
#
# - if defaults/main.yml points at an SFTPGo version that has never been
#   released, the release counter restarts at 0 (`v2.7.6-0`)
# - otherwise the counter is incremented (`v2.7.6-1`), but only if something
...
# "sftpgo", so every other change to the role went unreleased until a Renovate
# bump came along to carry it.
...
# Anchored on `sftpgo_version:` so that neither `sftpgo_container_image_tag`
# nor `sftpgo_container_image_self_build_repo_version`, both of which are
# derived from it through Jinja, can be mistaken for it. Those derived values
# are what a raw `sed` would happily return as `{{ sftpgo_version }}`.
version="$(sed -nE 's|^sftpgo_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the SFTPGo version from $defaults_path"
	exit 1
fi

# SFTPGo's version values carry a leading `v` (`v2.7.5`) and so do the tags
# (`v2.7.5-0`). Stripping any `v` before prepending one keeps this correct
